...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		WhileStmt st=nd.getEnclosingLoop();
		Unit breakPoint=breakTargets.get(st);			
		units.add(j.newGotoStmt(breakPoint));
		//units.add(breakPoint);
	...
	public Void visitIfStmt(IfStmt nd) {
		...
		units.add(join);//unit is the list that contains all the stmts
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		...
		units.add(label1);//unit is the list that contains all the stmts
